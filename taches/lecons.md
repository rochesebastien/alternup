...
### 2026-09-01 — `data` de `useFetch` est un `shallowRef` : muter un objet en place ne rafraîchit rien

**Contexte :** Après un glisser-déposer dans le nouveau calendrier, le `PUT` passait, le toast « Événement déplacé » s'affichait, mais le bloc restait à son ancienne place jusqu'au rechargement de la page. Typecheck, tests et build verts ; seul le test réel en navigateur (Playwright, mesure du `boundingBox` avant/après) l'a révélé.
