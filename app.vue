...
const layoutName = computed<'public' | 'tuteur' | 'alternant' | 'default'>(() => {
  ...
  if (path === '/tuteur' || path.startsWith('/tuteur/')) return 'tuteur'
  if (path === '/alternant' || path.startsWith('/alternant/')) return 'alternant'
  if (PUBLIC_LAYOUT_PAGES.includes(path)) return 'public'
